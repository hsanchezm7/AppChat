...
public class AñadirContactoExistente extends JDialog {
	...
	/**
	 * Crea un diálogo para añadir un contacto existente del que ya se conoce el
	 * número. Permite asignar un nombre personalizado al contacto y marcarlo como
	 * añadido manualmente.
	 *
	...
	public AñadirContactoExistente(Window owner, String phone) {
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (name.isEmpty()) {
					JOptionPane.showMessageDialog(AñadirContactoExistente.this, "El campo 'Name' no puede estar vacío.",
							"Advertencia", JOptionPane.WARNING_MESSAGE);
					return; // Salir del método si no pasa la validación
				}

				if (phone.equals(AppChat.getInstance().getCurrentUser().getPhone())) {
					JOptionPane.showMessageDialog(AñadirContactoExistente.this, "No puedes añadirte como contacto.",
							"Advertencia", JOptionPane.WARNING_MESSAGE);
					return;
				}
				...
				ContactoIndividual contactoExistente = AppChat.getInstance().getCurrentUser().getContactos().stream()
						...
						.filter(c -> c.getMovil().equals(phone)).findFirst().orElse(null);

				boolean operacionExitosa;

				if (contactoExistente != null) {
					// Si el contacto ya existe, actualizar su nombre y marcarlo como añadido
					// manualmente
					...
					if (operacionExitosa) {
						JOptionPane.showMessageDialog(AñadirContactoExistente.this,
								"Contacto actualizado correctamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
					}
				} else {
					...
					if (operacionExitosa) {
						JOptionPane.showMessageDialog(AñadirContactoExistente.this, "Contacto añadido correctamente.",
