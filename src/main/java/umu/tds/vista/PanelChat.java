...
public class PanelChat extends JPanel {
	...
	public void cargarConversacion(Contacto contacto) {
		...
		mensajes.stream().map(m -> {
			if (controlador.esMensajeDelUsuarioActual(m)) {
				...
				if (m.getEmoticono() >= 0) {
					return new BubbleText(panelMensajes, m.getEmoticono(), Color.GREEN, contacto.getNombre(),
							BubbleText.RECEIVED, 18);
				} else {
					return new BubbleText(panelMensajes, m.getTexto(), Color.GREEN, contacto.getNombre(),
							BubbleText.RECEIVED);
				}
			}
	...
	private void enviarMensajeTexto() {
		if (contactoActual == null) {
			JOptionPane.showMessageDialog(this, "Seleccione un contacto para enviar el mensaje", "Error",
		...
		if (enviado) {
			BubbleText burbuja = new BubbleText(panelMensajes, mensajeTextField.getText(), Color.GREEN, "Yo",
					BubbleText.SENT);
	...
	private void enviarMensajeEmoji(int emojiId) {
		if (contactoActual == null) {
			JOptionPane.showMessageDialog(this, "Seleccione un contacto para enviar el emoji", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
